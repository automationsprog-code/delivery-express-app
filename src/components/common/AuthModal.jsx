...
export default function AuthModal({ onClose, defaultTab = 'customer' }) {
  ...
  // Handle Google Login Form Submission (Graceful, seamless Google sign in)
        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {selectedRole === 'customer' && (
            <div className="space-y-4">
              ...
              {/* If in Google Prompt Mode */}
              {customerMode === 'google_prompt' ? (
                <form onSubmit={handleGoogleSubmit} className="space-y-3.5 bg-slate-50 dark:bg-zinc-950/80 p-4 rounded-3xl border border-slate-200 dark:border-zinc-800 animate-fadeIn text-xs">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">Sign In with Google Account</h4>
                    </div>
                    <button type="button" onClick={() => setCustomerMode('signup')} className="text-slate-400 hover:text-slate-600">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 rounded-2xl px-3.5 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-rose-500 font-medium"
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 text-white font-black rounded-2xl text-xs sm:text-sm shadow-md flex items-center justify-center gap-2"
                  >
                    <span>Authorize with Google Email</span>
                    <ArrowRight className="w-4 h-4" />
                  </button>
                </form>
              ) : (
                /* Google Account Picker Trigger */
                <button
                  type="button"
                  onClick={() => { setCustomerMode('google_prompt'); setErrorMsg(''); }}
